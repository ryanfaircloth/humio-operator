...
func (r *HumioClusterReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	for _, pool := range humioNodePools {
		_, err = constructPod(pool, "", &podAttachments{})
		if err != nil {
	...
	for _, pool := range humioNodePools {
		result, err = r.ensureHumioServiceAccountAnnotations(ctx, hc, pool)
		if result != emptyResult || err != nil {
			return result, err
		}
	}
	...
	for _, pool := range humioNodePools {
		err = r.ensureServiceExists(ctx, hc, pool)
		if err != nil {
			return reconcile.Result{}, err
		}
	}
	...
	for _, pool := range humioNodePools {
		err = r.ensureHumioPodPermissions(ctx, hc, pool)
		if err != nil {
			return reconcile.Result{}, err
		}
	}
	...
	for _, pool := range humioNodePools {
		err = r.ensureInitContainerPermissions(ctx, hc, pool)
		if err != nil {
			return reconcile.Result{}, err
		}
	}
	...
	for _, pool := range humioNodePools {
		err = r.ensureAuthContainerPermissions(ctx, hc, pool)
		if err != nil {
			return reconcile.Result{}, err
		}
	}
	...
	for _, pool := range humioNodePools {
		err = r.ensureHumioNodeCertificates(ctx, hc, pool)
		if err != nil {
	...
	for _, pool := range humioNodePools {
		result, err = r.ensurePersistentVolumeClaimsExist(ctx, hc, pool)
		if result != emptyResult || err != nil {
			return result, err
		}
	}
	...
	for _, pool := range humioNodePools {
		result, err = r.ensurePodsExist(ctx, hc, pool)
		if result != emptyResult || err != nil {
			return result, err
		}
	}
	...
	defer func(ctx context.Context, humioClient humio.Client, hc *humiov1alpha1.HumioCluster) {
		...
		for _, pool := range humioNodePools {
			_ = r.setPod(ctx, hc, pool)
		}
	}(ctx, r.HumioClient, hc)

	for _, pool := range humioNodePools {
		err = r.ensureLabels(ctx, cluster.Config(), req, hc, pool)
		if err != nil {
			return reconcile.Result{}, err
		}
	}
	...
	for _, pool := range humioNodePools {
		...
		podsStatus, err := r.getPodsStatus(pool, foundPodList)
		if err != nil {
...
func (r *HumioClusterReconciler) setImageFromSource(ctx context.Context, hnp *HumioNodePool) error {
	if hnp.GetImageSource() != nil {
		...
		if err != nil {
			if errors.IsNotFound(err) {
				return fmt.Errorf("imageSource was set but no configMap exists by name %s in namespace %s", hnp.GetImageSource().ConfigMapRef.Name, hnp.GetNamespace())
			}
			return fmt.Errorf("unable to get configMap with name %s in namespace %s", hnp.GetImageSource().ConfigMapRef.Name, hnp.GetNamespace())
		}
		if imageValue, ok := configMap.Data[hnp.GetImageSource().ConfigMapRef.Key]; ok {
			hnp.SetImage(imageValue)
		} else {
			return fmt.Errorf("imageSource was set but key %s was not found for configmap %s in namespace %s", hnp.GetImageSource().ConfigMapRef.Key, hnp.GetImageSource().ConfigMapRef.Name, hnp.GetNamespace())
		}
	}
	return nil
}
...
func (r *HumioClusterReconciler) ensureHumioPodPermissions(ctx context.Context, hc *humiov1alpha1.HumioCluster, hnp *HumioNodePool) error {
	...
	err := r.ensureServiceAccountExists(ctx, hc, hnp, hnp.GetHumioServiceAccountName(), hnp.GetHumioServiceAccountAnnotations())
	if err != nil {
	...
	if helpers.IsOpenShift() {
		err = r.ensureSecurityContextConstraintsContainsServiceAccount(ctx, hnp.GetNamespace(), hnp.GetHumioServiceAccountName())
		if err != nil {
...
func (r *HumioClusterReconciler) ensureInitContainerPermissions(ctx context.Context, hc *humiov1alpha1.HumioCluster, hnp *HumioNodePool) error {
	...
	if hnp.InitServiceAccountIsSetByUser() {
		...
		err := r.ensureServiceAccountSecretExists(ctx, hc, hnp, hnp.GetInitServiceAccountSecretName(), hnp.GetInitServiceAccountName())
		if err != nil {
	...
	err := r.ensureServiceAccountExists(ctx, hc, hnp, hnp.GetInitServiceAccountName(), map[string]string{})
	if err != nil {
	...
	err = r.ensureInitClusterRole(ctx, hnp)
	if err != nil {
	...
	// This should be namespaced by the name, e.g. clustername-namespace-name
	// Required until https://github.com/kubernetes/kubernetes/issues/40610 is fixed
	err = r.ensureInitClusterRoleBinding(ctx, hnp)
	if err != nil {
	...
	// In cases with OpenShift, we must ensure our ServiceAccount has access to the SecurityContextConstraint
	if helpers.IsOpenShift() {
		err = r.ensureSecurityContextConstraintsContainsServiceAccount(ctx, hnp.GetNamespace(), hnp.GetInitServiceAccountName())
		if err != nil {
			r.Log.Error(err, "could not ensure SecurityContextConstraints contains ServiceAccount")
			return err
		}
...
func (r *HumioClusterReconciler) ensureAuthContainerPermissions(ctx context.Context, hc *humiov1alpha1.HumioCluster, hnp *HumioNodePool) error {
	...
	if hnp.AuthServiceAccountIsSetByUser() {
		...
		// https://github.com/kubernetes/kubernetes/issues/66020#issuecomment-590413238
		err := r.ensureServiceAccountSecretExists(ctx, hc, hnp, hnp.GetAuthServiceAccountSecretName(), hnp.GetAuthServiceAccountName())
		if err != nil {
	...
	err := r.ensureServiceAccountExists(ctx, hc, hnp, hnp.GetAuthServiceAccountName(), map[string]string{})
	if err != nil {
	...
	err = r.ensureAuthRole(ctx, hc, hnp)
	if err != nil {
		r.Log.Error(err, "unable to ensure auth role exists")
		return err
	}

	err = r.ensureAuthRoleBinding(ctx, hc, hnp)
	if err != nil {
	...
	// In cases with OpenShift, we must ensure our ServiceAccount has access to the SecurityContextConstraint
	if helpers.IsOpenShift() {
		err = r.ensureSecurityContextConstraintsContainsServiceAccount(ctx, hnp.GetNamespace(), hnp.GetAuthServiceAccountName())
		if err != nil {
			r.Log.Error(err, "could not ensure SecurityContextConstraints contains ServiceAccount")
			return err
		}
...
func (r *HumioClusterReconciler) ensureInitClusterRole(ctx context.Context, hnp *HumioNodePool) error {
	...
	if err != nil {
		if errors.IsNotFound(err) {
			clusterRole := kubernetes.ConstructInitClusterRole(clusterRoleName, hnp.GetNodePoolName())
...
func (r *HumioClusterReconciler) ensureLabels(ctx context.Context, config *humioapi.Config, req reconcile.Request, hc *humiov1alpha1.HumioCluster, hnp *HumioNodePool) error {
	...
	for idx, pod := range foundPodList {
		...
		if kubernetes.LabelListContainsLabel(pod.GetLabels(), kubernetes.NodeIdLabelName) {
			if hnp.PVCsEnabled() {
				err := r.ensurePvcLabels(ctx, hc, hnp, pod, pvcList)
				if err != nil {
...
func (r *HumioClusterReconciler) ensurePvcLabels(ctx context.Context, hc *humiov1alpha1.HumioCluster, hnp *HumioNodePool, pod corev1.Pod, pvcList []corev1.PersistentVolumeClaim) error {
...
func (r *HumioClusterReconciler) ensureHumioServiceAccountAnnotations(ctx context.Context, hc *humiov1alpha1.HumioCluster, hnp *HumioNodePool) (reconcile.Result, error) {
...
func (r *HumioClusterReconciler) ensurePersistentVolumeClaimsExist(ctx context.Context, hc *humiov1alpha1.HumioCluster, hnp *HumioNodePool) (reconcile.Result, error) {
	if !hnp.PVCsEnabled() {
